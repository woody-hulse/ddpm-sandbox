"""
Graph encoder auxiliary task comparison.
...
Usage:
    ...
    python -m tools.analysis.graph_aux
    ...
    # skip branches
    python -m tools.analysis.graph_aux --skip-diffae-mlp --skip-encoder
    python -m tools.analysis.graph_aux --skip-graphae-mlp --skip-diffae-mlp

    # keep encoder frozen (linear-probe mode)
    python -m tools.analysis.graph_aux --freeze-encoder
"""
...
from data_loader import OnlineMSBatcher
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=str, default="figures/graph_aux",
    ...
    cfg = get_config() if args.latent_dim is None else get_config(latent_dim=args.latent_dim)
