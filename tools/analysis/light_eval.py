...
variants and writes every artifact under `figures/light_eval/` by default.
...
DIAGNOSE_DIR = os.path.join(REPO_ROOT, "diagnose")
FULL_PMT_XY_PATH = os.path.join(REPO_ROOT, "data", "pmt_xy.h5")
...
from tools.analysis.compare_rqs import collect_rqs, compute_rqs, plot_rq_comparison, wf_to_z_profile
...
from tools.analysis.eval_recon import (
...
from tools.visualization.plot_rq_distributions import plot_distributions
from tools.visualization.plot_real_event_3d import plot_waveform_3d_scatter
...
def validate_shared_sample_store(
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Shared sample cache not found at {path}. Run `python -m tools.analysis.light_eval --regenerate` to create it."
        )
...
def plot_continuous_embedding_panels(
    ...
    from tools.visualization.plot_umap import knn_label_smoothness
...
def run_latent_delta_mu(
    ...
    from tools.visualization.plot_umap import REDUCERS
...
def run_ss_ms_probe(
    ...
    from diagnose.probe_ss_ms import generate_ms_coadded, generate_ss_coadded, plot_probe_bar, plot_scatter, reduce, train_probe
...
def run_anomaly_probe(
    ...
    from tools.analysis.anomaly_probe import (
        ANOMALY_TYPES,
        anomaly_heatmap,
        anomaly_scores,
        fit_reduce,
...
def skipped_legacy_experiments() -> Dict[str, str]:
    return {
        ...
        "tools.visualization.view_events": "Covered by the reconstruction example panels emitted by the RQ and reconstruction evaluations.",
    }
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-dir", type=str, default="figures/light_eval")
...
def main() -> None:
    ...
    output_root = ensure_dir(os.path.join(REPO_ROOT, args.output_dir))
