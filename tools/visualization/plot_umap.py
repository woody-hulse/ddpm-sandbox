...
Usage:
    ...
    python -m tools.visualization.plot_umap --latent-dim 64 --method tsne
    python -m tools.visualization.plot_umap --latent-dim 64 --lopsided
    python -m tools.visualization.plot_umap --latent-dim 64 --lopsided --lopsided-frac 0.3
"""
import os
import argparse
...
def _waveform_roughness(batch_np: np.ndarray, n_channels: int, n_time: int) -> np.ndarray:
    ...
    """
    from tools.analysis.compare_rqs import wf_to_z_profile
    B = batch_np.shape[0]
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="figures/umap")
    ...
    if args.lopsided:
        ...
        if not panels:
            print("No encoded latents found. Run `python -m tools.analysis.compare_latent_sizes` first.")
            return
