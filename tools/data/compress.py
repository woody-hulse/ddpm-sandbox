...
Usage examples:
    ...
    python -m tools.data.compress --channels 42 --time-window 500

    # Single-node mode (sum all channels)
    python -m tools.data.compress --channels 1

    # Generate MS events from existing SS data
    python -m tools.data.compress --ms --n-ms-events 10000 --delta-min -30 --delta-max 30

    # Full pipeline: compress then generate MS
    python -m tools.data.compress --channels 42 --time-window 500 --ms --n-ms-events 10000
"""
import argparse
...
def _compression_figure_dir(out_dir: Path, tritium_in: Path) -> Path:
    figure_dir = Path("figures") / "data_compression" / out_dir.name / tritium_in.stem
...
def compress_select_channels(
    *,
    ...
    out_dir = Path(out_dir) if out_dir is not None else tritium_in.parent
    out_dir.mkdir(parents=True, exist_ok=True)
    figure_dir = _compression_figure_dir(out_dir, tritium_in)

    n = int(n_channels)
    tritium_out = out_dir / f"{tritium_in.stem}_c{n}_t{int(time_window)}.h5"
    pmt_xy_out = out_dir / f"pmt_xy_c{n}.h5"
    plot_out = figure_dir / f"selected_pmts_c{n}_t{int(time_window)}.png"
    ...
    with h5py.File(tritium_in, "r") as fin:
        ...
        k = int(sample_events)
        if k > 0 and s_out > 0:
            sample_dir = figure_dir / f"sample_events_c{n}_t{tw}"
...
Examples:
  ...
  python -m tools.data.compress --ms -i data/tritium_ss_c42_t500.h5 -o data/tritium_ms_c42.h5
